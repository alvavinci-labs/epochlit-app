...
-- stories: 誰でも読める（preview・image_url・title 等）
-- body（全文）はサーバーサイドでアクセス制御するため、
-- anon キーでは body を除いたカラムのみ SELECT できるビューを作る
...
-- anon: stories_public ビューを読める
CREATE POLICY "anon can read stories_public" ON stories
